...
In the program, I have used time delays in three functions: in timer (displaying the two-digit number within one second), the run/pause function, and the debouncing.

For the timer, I used 10000000 time constant, 3 in run/pause, and 50000 in debouncing.
...
For the pause function, I have implemented time delay and repeated checking of a change in the state of the key. The delay is important to ensure that the state of the input is already stable.

Without the time delays, may still perform the functions, however, the counting would be too fast and will not be observable. 
